...
export async function POST(request: Request) {
  ...
  try {
    const data = await request.json() as { kind?: ConfigKind; value?: unknown; changeNote?: string };
    if (!data.kind) return NextResponse.json({ error: "編集対象が不正です。" }, { status: 400 });
    ...
    if (data.kind === "tools") {
      ...
      contentFiles = buildToolContentChanges((await getContentAdminSnapshot()).toolPaths, parsed.data);
