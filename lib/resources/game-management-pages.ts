...
const jaSaveDataSchemaTexts: SaveDataSchemaManagerTexts = {
  hero: {
    ...
    title: 'SaveData スキーマ管理',
    description: 'SaveDataFieldDefinitions、SaveDataFieldOptions、SaveDataFieldOverrides を管理し、公開 schema API で解決される最終形も同画面で確認できます。共有選択肢セットの管理は専用画面で行います。',
  },
  common: {
    actionsColumn: '操作',
    close: '閉じる',
  ...
  buttons: {
    ...
    assignSelectedSchemas: '選択した schema を採用',
    definitionDetail: '詳細',
    definitionEdit: '編集',
    assign: '採用',
  ...
  sections: {
    catalog: {
      ...
      emptyMessage: 'schema カタログがありません。',
    },
    assignedDefinitions: {
      title: '採用項目',
  ...
  dialogs: {
    assignment: {
      updateTitle: '採用設定を更新',
      createTitle: 'schema をゲームソフト分類へ採用',
      unassignTitle: 'schema の採用を解除',
      updateSummary: '選択した schema {schemaCount} 件の採用設定を、ゲームソフト分類 {contentGroupCount} 件で更新します。',
      createSummary: '選択した schema {schemaCount} 件を、ゲームソフト分類 {contentGroupCount} 件へ採用します。',
      unassignSummary: '選択した schema {schemaCount} 件を、現在のゲームソフト分類から採用解除します。',
  ...
  messages: {
    ...
    duplicateFieldKey: 'fieldKey が重複しています。SaveData schema 全体で一意になるよう、別の fieldKey を入力してください。',
    duplicateDisplayOrder: '表示順が重複しています。選択中のゲームソフト分類で重複しない表示順にしてください。',
    gameSoftwareCategoryRequired: 'ゲームソフト分類を選択してください。',
    assignmentAlreadyExists: '次のゲームソフト分類では既に採用済みの schema が含まれています: {gameSoftwareCategories}。採用設定の更新は「採用項目」一覧から行ってください。',
    assignmentApiUnavailable: '採用設定 API が利用できません。バックエンドに SaveDataFieldDefinitionAssignments の PUT / DELETE 実装が反映されているか確認してください。',
    selectCatalogToAssign: '採用する schema カタログを選択してください。',
    definitionRequiredFields: 'fieldKey と表示名は必須です。',
    noSchemaToAssign: '採用する schema がありません。',
    ...
    assignLoading: 'schema をゲームソフト分類へ採用中...',
    deleteAssignmentLoading: 'schema の採用を解除中...',
    ...
    createDefinitionSuccess: 'グローバル定義を作成しました。ゲームソフト分類へ反映するには schema カタログから採用してください。',
    ...
    deleteAssignmentSuccess: 'schema {schemaCount} 件の採用を解除しました。',
    assignmentDeletePartialFailure: '{reason} 採用解除に失敗した schema: {failedSchemas}',
